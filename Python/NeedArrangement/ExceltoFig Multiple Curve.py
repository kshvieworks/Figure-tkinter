...
# legendinfo = ["Time Response"]
# legendinfo = ["$\\sigma_{Total}$", "$\\sigma_{Outlier}$", "$\\sigma_{Frame}$", "$\\sigma_{Line}$", "$\\sigma_{y}$"]
# legendinfo = ['$\\sigma_{y, Image}$', '$\\sigma_{y, Leakage}$']
# legendinfo = [f"Vieworks PIN Photodiode", f"Dongwoo FineChem OPD"]
# legendinfo = ["Transmittance of Silicon"]
# legendinfo = ["Time Response @ V = -2V"]
# legendinfo = ["0.2 FPS", "0.5 FPS", "1 FPS", "2 FPS", "5 FPS"]
# legendinfo = [f'60$\\mu$Gy', f'120$\\mu$Gy', f'240$\\mu$Gy', f'480$\\mu$Gy']
# legendinfo = [f'Type II-2']
# legendinfo = [f'']
legendinfo = ["V$_{EXT}$ = -2.0V", "V$_{EXT}$ = -1.5V", "V$_{EXT}$ = -1.0V", "V$_{EXT}$ = -0.5V"]
# legendinfo = ["Photodiode", 'Image Sensor']
# legendinfo = ["", "DRZ-Plus", "Sample #1", "Sample #2"]

# legendinfo = ["80mA", "40mA", '20mA', '10mA']
# legendinfo = ["V$_{COM}$ = 0V", "V$_{COM}$ = -1.6V"]
...
class ClipboardtoFig:
    ...
    def DrawCurve(self):
        ...
        #     np.append(self.data1, data[1][yidx])
        # self.TextatTargetPos(xval, data)
    ...
    def PaintFace(self):
        ...
        self.drawax.axvspan(x-25, x+25, facecolor='g', alpha=0.2)
