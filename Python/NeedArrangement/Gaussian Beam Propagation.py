...
class GaussianBeamPath:
    ...
    def __main__(self):
        ...
        thetaLable = Label(self.OutputFrame, width=14,  text="theta [\N{DEGREE SIGN}]", relief="ridge", font=(f"{tkfont['Font']} {tkfont['FontSize']}"))
        thetaLable.grid(row=3, column=0, padx=1, pady=1)
        ...
        self.thetaVal = Entry(self.OutputFrame, width=8, textvariable="", relief="ridge", font=(f"{tkfont['Font']} {tkfont['FontSize']}"))
        self.thetaVal.grid(row=3, column=1, padx=1, pady=1)
        self.thetaVal.insert(0, 0)

        NewFigure = Button(self.OutputFrame, width=20, height=2, text="Draw Beam Path", relief="raised", font=(f"{tkfont['Font']} {tkfont['FontSize']}"), command=self.NewFig)
        NewFigure.grid(row=1, column=2, rowspan=3, padx=2, pady=2)
    ...
    def DrawFig(self, fig, ax):
        ...
        zstep, rstep, theta = self.getParams()
        z = np.arange(DefaultInfos['xLim'][0], DefaultInfos['xLim'][1], zstep)
        r = np.arange(DefaultInfos['yLim'][0], DefaultInfos['yLim'][1], rstep)

        data = self.GaussainBeamPropagation(z, r, theta)
        ...
        c = ax.imshow(data, cmap=colorstyle, alpha=alphavalue,
                      ...
                      origin='lower',
                      norm = SymLogNorm(linthresh = np.power(10,float(-linth)), vmin=DefaultInfos['CMapMin'], vmax=DefaultInfos['CMapMax'], base=10))
    ...
    def getParams(self):
        zstep = float(self.xStepVal.get())
        rstep = float(self.yStepVal.get())
        theta = np.deg2rad(float(self.thetaVal.get()))

        return zstep, rstep, theta
