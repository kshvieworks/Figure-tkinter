...
legendinfo = ["Bulk Current Density", "Surface Current Density", "Lateral Current Density"]
...
class ClipboardtoFig:
    ...
    def DrawCurve(self):
        ...
        pos = 0.5+np.arange(0, len(data), 1)
        bottom = np.zeros(len(data))

        for k in range(1, np.shape(data)[1]):
            c = next(self.color)
            self.drawax.bar(pos, data[k], bottom=bottom, width=0.45, color=c)
            bottom += data[k]
        self.drawax.set_xticks(pos)
        self.drawax.set_xticklabels(data[0], fontsize=tickfontstyle['FontSize'])

        self.drawax.legend(legendinfo, loc='best', fontsize=tickfontstyle['FontSize']-4)
        plt.pause(0.001)
