...
class ClipboardtoFig:
    ...
    def DrawCurve(self):
        ...
        params = f"$\mathit{{\mu}}={np.round(mu, 1)}, \mathit{{\sigma}}={np.round(sigma, 1)} ({x_data.__len__()} pts)$"
        params_effective = f"$\mathit{{\mu}}={np.round(mu_effective, 1)}, \mathit{{\sigma}}={np.round(sigma_effecitve, 1)} ({x_effective_data.__len__()} pts)$"
        params_outlier = f"$\mathit{{\mu}}={np.round(mu_outlier, 1)}, \mathit{{\sigma}}={np.round(sigma_outlier, 1)} ({x_outlier_data.__len__()} pts)$"

        # self.drawax.plot(x, mean, c=c, alpha=alphavalue,  marker='o', linestyle='None')
        self.drawax.plot(x_data, y_data, 'ro', markersize = 3, mfc = 'None', alpha=alphavalue, linestyle = 'None', label = params)
        self.drawax.plot(x_effective_data, y_effective_data, 'go', markersize = 3, mfc = 'None', alpha=alphavalue, linestyle = 'None', label = params_effective)
        self.drawax.plot(x_outlier_data, y_outlier_data, 'bo', markersize = 3, mfc = 'None', alpha=alphavalue, linestyle = 'None', label = params_outlier)

        self.drawax.errorbar(x_data[0], mu, yerr=sigma, fmt='Dk', ecolor = 'k', capsize=10)
        self.drawax.errorbar(x_effective_data[0], mu_effective, yerr=sigma_effecitve, fmt='Dk', ecolor = 'k', capsize=10)
        self.drawax.errorbar(x_outlier_data[0], mu_outlier, yerr=sigma_outlier, fmt='Dk', ecolor = 'k', capsize=10)

        # self.drawax.scatter(x_effective_data, y_effective_data, 'g', alpha=alphavalue)
        # self.drawax.scatter(x_outlier_data, y_outlier_data, 'b', alpha=alphavalue)

        # self.drawax.plot(data[0], data[1], c=c, alpha=alphavalue, label=l, marker='o')
        self.drawax.set_xticklabels(['', 'Total', 'w/o Outlier', 'Outlier'])
