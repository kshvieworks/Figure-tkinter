...
class ClipboardtoFig:
    ...
    def DrawGaussianCurve(self):
        ...
        Initial_Amplitude, Initial_Mean, Initial_Stddev = max(calibrated_y), self.data[0][np.argmax(self.data[1])], 10
        ...
        params = f"$\mathit{{\mu}}={int(popt[1])}, \mathit{{\sigma}}={int(popt[2])}$"

        self.Gaussian_FWHM(fit_data, x)

        self.drawax.plot(x, fit_data, 'b', alpha=0.7*alphavalue, label=params)
        self.drawax.legend(loc='best', fontsize=tkfont['FontSize'])
        plt.pause(0.001)
